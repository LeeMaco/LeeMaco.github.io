document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadSettings() {
        ...
        if (backupTypeSelect) {
            backupTypeSelect.value = settings.backupType || 'full';
        }
        
        // 顯示最後備份信息
        const lastBackupInfo = JSON.parse(localStorage.getItem('lastBackupInfo') || 'null');
        if (lastBackupInfo) {
            const backupInfoDiv = document.createElement('div');
            backupInfoDiv.className = 'backup-info';
            backupInfoDiv.textContent = `上次備份: ${new Date(lastBackupInfo.timestamp).toLocaleString()} (${lastBackupInfo.type === 'full' ? '完整備份' : '增量備份'})`;
            
            const backupSettingsDiv = document.querySelector('.backup-settings');
            if (backupSettingsDiv) {
                backupSettingsDiv.appendChild(backupInfoDiv);
            }
        }
    }
    ...
    function saveSettings() {
        ...
        const settings = {
            ...
            backupType: backupTypeSelect ? backupTypeSelect.value : 'full'
        };
        ...
        const lastBackupInfo = JSON.parse(localStorage.getItem('lastBackupInfo') || 'null');
        if (lastBackupInfo) {
            const existingInfoDiv = document.querySelector('.backup-settings .backup-info:last-child');
            if (existingInfoDiv && existingInfoDiv.textContent.includes('上次備份')) {
                existingInfoDiv.textContent = `上次備份: ${new Date(lastBackupInfo.timestamp).toLocaleString()} (${lastBackupInfo.type === 'full' ? '完整備份' : '增量備份'})`;
            }
        }
    }
    ...
    function backupData() {
        // 獲取備份類型
        const backupTypeSelect = document.getElementById('backup-type');
        const backupType = backupTypeSelect ? backupTypeSelect.value : 'full';
        ...
        if (window.backupSystem && typeof window.backupSystem.performBackup === 'function') {
            // 使用增強的備份系統
            window.backupSystem.performBackup(backupType);
        } else {
            // 舊的備份邏輯作為後備
            if (email) {
        ...
        localStorage.setItem('lastBackupInfo', JSON.stringify(lastBackupInfo));
    }
