...
const BookData = {
    ...
    loadBooksFromJSON: function() {
        ...
        return Promise.any(fetchPromises)
            ...
            .catch(error => {
                ...
                // 自動備份設置
    backupSettings: {
        enabled: localStorage.getItem('backupEnabled') === 'true',
        interval: parseInt(localStorage.getItem('backupInterval') || '24'),
        lastBackupTime: localStorage.getItem('lastBackupTime') || null,
        
        // 設置備份選項
        setSettings: function(enabled, interval) {
            this.enabled = enabled;
            this.interval = interval;
            localStorage.setItem('backupEnabled', enabled);
        ...
        performBackup: function() {
            try {
                const backupData = {
                    ...
                    books: BookData.jsonBooks,
                    settings: {
                        backupEnabled: this.enabled,
                        backupInterval: this.interval
                    }
                };
                localStorage.setItem('books_backup', JSON.stringify(backupData));
                this.lastBackupTime = backupData.timestamp;
                localStorage.setItem('lastBackupTime', this.lastBackupTime);
                console.log('已創建本地數據備份');
                return true;
            } catch (backupError) {
                console.error('創建備份失敗:', backupError);
                return false;
            }
        },
        ...
        shouldBackup: function() {
            ...
            return hoursDiff >= this.interval;
        },
        
        // 初始化備份定時器
        initBackupTimer: function() {
            if (this.enabled) {
                setInterval(() => {
    ...
    try {
        const backupData = {
            timestamp: new Date().toISOString(),
            error: errorMessage,
            lastSuccessfulData: this.jsonBooks.length > 0 ? this.jsonBooks : null
        };
        localStorage.setItem('books_backup', JSON.stringify(backupData));
        console.log('已創建本地數據備份');
    } catch (backupError) {
